...
> void DilateFilter<PixelType>::applyToDiamond(const Image::Image<PixelType> & srcImage,Image::Image<PixelType> & dstImage) const {
	...
	Algorithm::BaseAlgorithm3x3<DilateAlgorithm3x3Cross<PixelDataType>,PixelDataType>(srcImage.getDataView(),dstImage.getDataView());
	if(iterations==1){return;}

	Image::Image<PixelType> tempImage(dstImage.getSize(),dstImage.getDataManager());

	Algorithm::BaseAlgorithm3x3<DilateAlgorithm3x3Cross<PixelDataType>,PixelDataType>(dstImage.getDataView(),tempImage.getDataView());
	if(iterations==2){
