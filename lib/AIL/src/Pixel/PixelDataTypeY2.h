...
> class AIL_DLL_EXPORT PixelDataTypeY2 {
	public:
		...
		FINLINE explicit PixelDataTypeY2(){}
		FINLINE explicit PixelDataTypeY2(const NumberType & _scalar):x(_scalar) ,y(_scalar) ,z(_scalar){} //TODO: Is this correct? - I expect not , perhaps use (scalar,0,0) ?
		FINLINE explicit PixelDataTypeY2(const NumberType & _scalar1,const NumberType & _scalar2,const NumberType & _scalar3):x(_scalar1) ,y(_scalar2) ,z(_scalar3){}
		FINLINE          PixelDataTypeY2(const ThisType   & _pixel ):x(_pixel.x),y(_pixel.y),z(_pixel.z){}
		...
		template <typename DiffNumberType> FINLINE const void setComp(const PixelDataTypeY2<DiffNumberType,Yposition> & _pixel){x=_pixel.get<0>();y=_pixel.get<1>();z=_pixel.get<2>();};

		template <typename DiffNumberType> FINLINE PixelDataTypeY2<DiffNumberType,Yposition> getAsComp() const {return PixelDataTypeY2<DiffNumberType,Yposition>(x,y,z);};
	...
	//Operators
		...
		friend FINLINE ThisType operator-(const NumberType & scalar, const ThisType & pixel){return PixelDataTypeY2<NumberType,Yposition>(scalar)-pixel;}
		friend FINLINE ThisType operator+(const NumberType & scalar, const ThisType & pixel){return PixelDataTypeY2<NumberType,Yposition>(scalar)+pixel;}
		friend FINLINE ThisType operator*(const NumberType & scalar, const ThisType & pixel){return PixelDataTypeY2<NumberType,Yposition>(scalar)*pixel;}
		friend FINLINE ThisType operator/(const NumberType & scalar, const ThisType & pixel){return PixelDataTypeY2<NumberType,Yposition>(scalar)/pixel;}
